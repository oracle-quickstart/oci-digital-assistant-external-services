...
locals {
  app_name_normalized = replace(var.prefix_name, " ", "-")
}
...
resource "oci_identity_dynamic_group" "oda-dynamic-group" {
  ...
  description     = "${var.prefix_name} Dynamic Group"
  name            = "${local.app_name_normalized}DynamicGroup"
  matching_rule   = "any {all {instance.compartment.id='${var.compartment_ocid}'} }"
}
...
resource "oci_kms_vault" "oda-vault" {
  ...
  display_name = "${var.prefix_name} Vault"
  vault_type = "DEFAULT"
}
...
resource "oci_kms_key" "oda-key" {
  count = var.create_vault ? 1 : 0
  compartment_id = var.compartment_ocid
  display_name = "${var.prefix_name} Vault Encryption Key"
  key_shape {
    algorithm = "AES"
    length = 32
  }
...
resource "oci_identity_policy" "oda-policy" {
  provider       = oci.home
  compartment_id = var.compartment_ocid
  description    = "${var.prefix_name} Policies"
